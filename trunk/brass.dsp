...
deltaPressure = vgroup("Mouth Presure",mouthPressure - _) : 
				//lips are simulated by a biquad filter whose output is squared and hard-clipped
				vgroup("Mouth Piece",lipFilter(_*0.03) <: _*_ : clipingFunction);

bore(x) = deltaPressure(x) <: _*mouthPressure,(1-_)*x :> + ;

process = hgroup("STK Brass",
	(vgroup("[1]Bore Pressure",borePressure) <:
	vgroup("[2]Bore",bore) :
	vgroup("[3]Body Filter",dcblocker)) ~
	vgroup("[4]Bore Loop and Delay",boreDelay)
);