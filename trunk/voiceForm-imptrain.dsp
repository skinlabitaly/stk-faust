...
import("music.lib");
import("osc.lib"); // for imptrain = periodic impulse train
import("filter.lib"); // for lowpass3e = 3rd-order elliptic filter
...
//envelopes (declared in instrument.lib)
...
voiced = imptrain(freq+vibrato) : lowpass3e(3300)
//voiced = rdtable(impuls20TableSize, impuls20, int(phaser)) 
  : _*voiceGain*voicedEnvelope
	with{
		vibrato = osc(vibratoFreq)*vibratoGain*100*vibratoEnvelope;
		
		//readImpuls20 is declared in instrument.lib
