...
bilinear transform \cite{PASP}, energy conservation is only
approximately obtained, except when the allpass state variable happens
to be exactly zero when the coefficient is switched from $a_1$ to
$a_2$ or vice versa.
...
contain zero, and shortening the delay-line is typically a bit lossy
but never energy-creating.
...
A general property of allpass filters is that any delay element can be
replaced by an allpass to produce another allpass.  If the delay
element in Fig.{} \ref{nlf} is replaced by itself following another
first-order allpass of the form shown in Fig.{} \ref{nlf}, then the
second-order NLF allpass is obtained.
